...
if(isset($_FILES['audio']) && !empty($_FILES['audio']['tmp_name'])){
    ...
    $i = -1;
    while(true){
        if($i > -1) $filename = $i . "_" . $filename;
        if(is_file('./audio/' . $filename)) $i++;
        else break;
    }
    $file = $_FILES['audio']['tmp_name'];
    $type = mime_content_type($file);
    $type2 = $_FILES['audio']['type'];
    if(strpos($type, "audio/") !== false || strpos($type2, "audio/") !== false){
        $move = move_uploaded_file($file, './audio/' . $filename);
        if(!$move){
            $resp['status'] = 'failed';
            $resp['msg'] = 'Audio file upload failed';
...
// Run the DB save regardless of whether an image was uploaded
if(!isset($resp['status'])){
    ...
    if($save){
        ...
        // Image upload is optional — handled inside the save block
